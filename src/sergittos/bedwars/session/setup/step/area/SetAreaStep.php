...
abstract class SetAreaStep extends Step {
    ...
    public function onBlockInteract(Vector3 $touchVector, int $action, Cancellable $event, BedwarsItem $item): void {
        ...
        switch($action) {
            case PlayerInteractEvent::LEFT_CLICK_BLOCK:
                $this->firstPosition = $touchVector;
                $this->session->message("{GREEN}First position set.");
                break;
            case PlayerInteractEvent::RIGHT_CLICK_BLOCK:
                $this->secondPosition = $touchVector;
    ...
    public function onInteract(BedwarsItem $item): void {
        ...
        if(!isset($this->firstPosition) or !isset($this->secondPosition)) {
            $this->session->message("{RED}You must set both positions before setting the area.");
            return;
        }

        $this->setArea(new Area($this->firstPosition, $this->secondPosition));
